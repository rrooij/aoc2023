...
fn card_ranks() -> HashMap<char, u64> {
    HashMap::from([
        ('A', 13),
        ('K', 12),
        ('Q', 11),
...
impl Hand {
    ...
    fn get_card_map(cards: &Vec<char>) -> HashMap<char, u64> {
    ...
    fn get_score(mut cards: Vec<char>) -> Score {
        let rank_map = Self::get_card_map(&cards);
        let mut sorted_cards: Vec<(&char, &u64)> =
            rank_map.iter().filter(|(x, _)| **x != 'J').collect();
        sorted_cards.sort_by(|a, b| b.1.cmp(a.1));
        let highest_card = if !sorted_cards.is_empty() {
            sorted_cards[0].0
        } else {
            &'A'
        };
        ...
        let map = Self::get_card_map(&cards);
